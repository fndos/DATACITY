...
class APILightONViewSet(viewsets.ViewSet):
	...
	def list(self, request, date=None):
		...
		try:
			...
			qs = simulation_models.Vehicle.objects.raw('''SELECT * FROM simulation_vehicle as v INNER JOIN simulation_term as t ON v.term_id = t.id INNER JOIN simulation_gauging as g ON t.gauging_id = g.id WHERE g.date = %s AND v.type = 1 AND v.movement = 1''', [date]);
			qt = simulation_models.Term.objects.raw('''SELECT * FROM simulation_term as t INNER JOIN simulation_vehicle as v ON v.term_id = t.id INNER JOIN simulation_gauging as g ON t.gauging_id = g.id WHERE g.date = %s AND v.type = 1 AND v.movement = 1''', [date]);
			d = list(qs)
			t = list(qt)
...
class APILightOEViewSet(viewsets.ViewSet):
	...
	def list(self, request, date=None):
		...
		try:
			# Escoger solo en el rango de fechas determinado
			# Obtener la informacion de la BD
			qs = simulation_models.Vehicle.objects.raw('''SELECT * FROM simulation_vehicle as v INNER JOIN simulation_term as t ON v.term_id = t.id INNER JOIN simulation_gauging as g ON t.gauging_id = g.id WHERE g.date = %s AND v.type = 1 AND v.movement = 2''', [date]);
			qt = simulation_models.Term.objects.raw('''SELECT * FROM simulation_term as t INNER JOIN simulation_vehicle as v ON v.term_id = t.id INNER JOIN simulation_gauging as g ON t.gauging_id = g.id WHERE g.date = %s AND v.type = 1 AND v.movement = 2''', [date]);
			d = list(qs)
			t = list(qt)
...
class APILightNEViewSet(viewsets.ViewSet):
	...
	def list(self, request, date=None):
		...
		try:
			# Escoger solo en el rango de fechas determinado
			# Obtener la informacion de la BD
			qs = simulation_models.Vehicle.objects.raw('''SELECT * FROM simulation_vehicle as v INNER JOIN simulation_term as t ON v.term_id = t.id INNER JOIN simulation_gauging as g ON t.gauging_id = g.id WHERE g.date = %s AND v.type = 1 AND v.movement = 3''', [date]);
			qt = simulation_models.Term.objects.raw('''SELECT * FROM simulation_term as t INNER JOIN simulation_vehicle as v ON v.term_id = t.id INNER JOIN simulation_gauging as g ON t.gauging_id = g.id WHERE g.date = %s AND v.type = 1 AND v.movement = 3''', [date]);
			d = list(qs)
			t = list(qt)
...
class APIWeightONViewSet(viewsets.ViewSet):
	...
	def list(self, request, date=None):
		...
		try:
			# Escoger solo en el rango de fechas determinado
			# Obtener la informacion de la BD
			qs = simulation_models.Vehicle.objects.raw('''SELECT * FROM simulation_vehicle as v INNER JOIN simulation_term as t ON v.term_id = t.id INNER JOIN simulation_gauging as g ON t.gauging_id = g.id WHERE g.date = %s AND v.type = 2 AND v.movement = 1''', [date]);
			qt = simulation_models.Term.objects.raw('''SELECT * FROM simulation_term as t INNER JOIN simulation_vehicle as v ON v.term_id = t.id INNER JOIN simulation_gauging as g ON t.gauging_id = g.id WHERE g.date = %s AND v.type = 2 AND v.movement = 1''', [date]);
			d = list(qs)
			t = list(qt)
...
class APIWeightOEViewSet(viewsets.ViewSet):
	...
	def list(self, request, date=None):
		...
		try:
			# Escoger solo en el rango de fechas determinado
			# Obtener la informacion de la BD
			qs = simulation_models.Vehicle.objects.raw('''SELECT * FROM simulation_vehicle as v INNER JOIN simulation_term as t ON v.term_id = t.id INNER JOIN simulation_gauging as g ON t.gauging_id = g.id WHERE g.date = %s AND v.type = 2 AND v.movement = 2''', [date]);
			qt = simulation_models.Term.objects.raw('''SELECT * FROM simulation_term as t INNER JOIN simulation_vehicle as v ON v.term_id = t.id INNER JOIN simulation_gauging as g ON t.gauging_id = g.id WHERE g.date = %s AND v.type = 2 AND v.movement = 2''', [date]);
			d = list(qs)
			t = list(qt)
...
class APIWeightNEViewSet(viewsets.ViewSet):
	...
	def list(self, request, date=None):
		...
		try:
			# Escoger solo en el rango de fechas determinado
			# Obtener la informacion de la BD
			qs = simulation_models.Vehicle.objects.raw('''SELECT * FROM simulation_vehicle as v INNER JOIN simulation_term as t ON v.term_id = t.id INNER JOIN simulation_gauging as g ON t.gauging_id = g.id WHERE g.date = %s AND v.type = 2 AND v.movement = 3''', [date]);
			qt = simulation_models.Term.objects.raw('''SELECT * FROM simulation_term as t INNER JOIN simulation_vehicle as v ON v.term_id = t.id INNER JOIN simulation_gauging as g ON t.gauging_id = g.id WHERE g.date = %s AND v.type = 2 AND v.movement = 3''', [date]);
			d = list(qs)
			t = list(qt)
